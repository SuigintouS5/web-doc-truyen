...
class Truyen(models.Model):
    ...
    # THÊM MỚI: Định nghĩa các loại truyện
    ...
    collaborators = models.ManyToManyField(
        settings.AUTH_USER_MODEL,
        blank=True,
        related_name="truyen_collaborations"
    )
...
class Profile(models.Model):
    ...
    # --- THÊM MỚI CÁC TRƯỜNG LIÊN KẾT ---
    phone = models.CharField(max_length=15, blank=True, null=True)
    gmail = models.EmailField(max_length=255, blank=True, null=True)
    
    # Quyền riêng tư (Privacy)
    hide_phone = models.BooleanField(default=False)
    hide_gmail = models.BooleanField(default=False)

    def __str__(self):
        return f"Profile của {self.user.username}"

# --- THÊM MỚI MODEL MẠNG XÃ HỘI ---
class SocialLink(models.Model):
    # Liên kết với Profile thay vì User để đồng bộ cấu trúc mở rộng
    profile = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name='social_links')
    name = models.CharField(max_length=50) # Lưu: Facebook, TikTok, Zalo...
